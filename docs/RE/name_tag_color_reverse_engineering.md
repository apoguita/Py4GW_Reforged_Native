...
> table validated PASS across all living agents. Feature/usage guide: `docs/agent_name_tag_color.md`.
>
> **This doc covers LIVING AGENTS only.** Items and gadgets do **not** use the `GetConsiderColor`
> resolver - they have their own `GetTextData` and coloring mechanism. That RE now lives in a
> dedicated sibling doc: **`item_gadget_recolor_reverse_engineering.md`** (gadget hook EXE-pinned;
> item markup path fully RE'd). The §4 stub below is superseded by that doc.
...
> **Superseded / extended by `item_gadget_recolor_reverse_engineering.md`.** That doc carries the
> full item + gadget RE with EXE 06-14 pins (`CItemAgent::GetTextData` `FUN_007fa6a0`,
> `ItemCliGetCodedName` `FUN_0083f4f0`, `CGadgetAgent::GetTextData` `FUN_007f9950`) and the recolor
> levers. The summary below is retained for context.
