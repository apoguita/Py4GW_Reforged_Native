...
> **Location note.** This is the **native project's** local copy of the RE library
> (`Py4GW_Reforged_Native/docs/RE/`), mirrored from the Python project's `docs/RE/` so the C++ backend
> has the RE knowledge alongside the code that implements it. When the two copies diverge, the
> subsystem this repo actively works (e.g. `agent_recolor`, and the item/gadget recolor RE below) is
> authoritative here; keep general/UI RE in sync with the Python copy.

> **Backend note — we are on Reforged.** The current C++ backend is **this** project,
> `Py4GW_Reforged_Native`: migrated managers in `src\GW\<module>\` + `include\GW\<module>\`, runtime
> addresses resolved from `offsets\<module>.json`. It **replaces legacy GWCA**. Throughout these docs,
> GWCA names and `../Py4GW/vendor/gwca` paths are **legacy cross-references** (canonical nomenclature +
> how a subsystem worked pre-Reforged), not the source of truth for current code. `Gw.exe`/`Gw.wasm`
> addresses describe the actual game and stay valid.
...
| `name_tag_color_reverse_engineering.md` | **Living-agent** name-tag COLOR pipeline: the `GetConsiderColor` resolver (EXE `FUN_007f02e0`) + `AvCharGetConsiderColor` (`FUN_007d9cf0`) detour recipe/ABI, allegiance→ARGB table, proposed/shipped module, and the functional tests | Coloring **living-agent** name tags natively (per-agent/per-allegiance) — SHIPPED as `GW::agent_recolor` |
| `item_gadget_recolor_reverse_engineering.md` | **Item & gadget** name-tag COLOR pipeline: the per-view `GetTextData` structs, gadget inline `Color4b` hook (EXE `FUN_007f9950`), item ownership color + rarity `<c=@...>` markup path (`CItemAgent::GetTextData` `FUN_007fa6a0`, `ItemCliGetCodedName` `FUN_0083f4f0`), and the recolor levers | Coloring **item / gadget** name tags natively (extends the living-agent doc) |
